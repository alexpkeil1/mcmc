...
function gvs_logit(
    ...
    rr = hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, _gamma_store, _bg_store, _pi_store)
    nms = vcat(
        :chain,
        :iter,
        :m1,
        ...
        [Symbol("gamma" * "[$i]") for i = 1:p],
        [Symbol("b*g" * "[$i]") for i = 1:p],
        [Symbol("pi" * "[$i]") for i = 1:j],
    )
...
gvs_logit(y, X, Xint, pl, iter, burnin; thin = 1, chain = 1, offset = 0.0) = gvs_logit(
    y,
    X,
    Xint,
    pl,
...
gvs_logit(y, X, Xint, pl, iter; thin = 1, chain = 1, offset = 0.0) =
    gvs_logit(y, X, Xint, pl, iter, 0; thin = thin, chain = chain, offset = offset)
...
gvs_hlogit(y, X, Xint, pl, iter, burnin; thin = 1, chain = 1, nu = 1.0, offset = 0.0) = gvs_hlogit(
    y,
    X,
    Xint,
    pl,
...
gvs_hlogit(y, X, Xint, pl, iter; thin = 1, chain = 1, nu = 1.0, offset = 0.0) =
    gvs_hlogit(y, X, Xint, pl, iter, 0; thin = thin, chain = chain, nu = nu, offset = offset)
...
gvs_zlogit(y, X, Xint, pl, iter, burnin; thin = 1, chain = 1, G = Float64(size(y, 1)), offset = 0.0) = gvs_zlogit(
    y,
    X,
    Xint,
    pl,
...
gvs_zlogit(y, X, Xint, pl, iter; thin = 1, chain = 1, G = Float64(size(y, 1)), offset = 0.0) =
    gvs_zlogit(y, X, Xint, pl, iter, 0; thin = thin, chain = chain, G = G, offset = offset);
