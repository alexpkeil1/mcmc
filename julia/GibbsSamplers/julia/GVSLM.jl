...
function inits(rng, pl, iter, binits, y, X, Xint, _tau_eta0, _mu_tau0, _mu_eta0, _mu_mu0, _sigmaa0, pa, pb)
    ...
    _iLams = Diagonal(vcat(_tau_eta0[1]^(-2), [_mu_tau0[1]^(-2) for j = 1:p]))
    _muvec = vcat(_mu_eta0[1], [_mu_mu0[1] for j = 1:p])
    # storage
...
function sampleinvsigma2(rng, a, _sigmab0, res, y, X, _bg)
    ...
    _invsigma2 = rand(rng, Gamma(a, 1.0 / b))
end
...
function sampleinvsigma2_or(rng, a, _sigmab0, res, y, X, _bg; K = 3, _invsigma2 = _invsigma2)
    ...
    b = _sigmab0 + se / 2.0
    #sigma2 = rand(rng, InverseGamma(a, b))
    orcand = rand(rng, Gamma(a, 1.0 / b), K)
...
function sampleinvsigma2_or(rng, _sigmab0, res, y, X, _bg, nu, _invsigma2, Nf; K = 3)
    ...
    sa = (Nf + nu) / 2.0
    sb = nu / V + se / 2.0
    orcand = rand(rng, Gamma(sa, 1.0 / sb), K)
    sp = sortperm(vcat(_invsigma2, orcand))
    selidx = K - sp[1] + 2
    _invsigma2 = orcand[sp[2:end].==selidx][1]
end
...
function updategamma!(rng, _gamma, _pi, _beta, Nuj, halfinvsigma2, res, y, X, j, plidx, pl, pa, pb)
    ...
    @inbounds for q in rp                   # loop over hierarchical groups in random order
        ...
        for k in vp                               # loop over variables within group in random order
            ...
            pj = 1.0 / (1.0 + exp(dj - cj))       # C*(lik*prior inclusion)/[C*(lik*prior exclusion) + C*(lik*prior exclusion) ] = posterior probability of inclusion
...
function gvs_lm(
    ...
    chain = 1,
    _mu_eta0 = 0.0,
    _tau_eta0 = 10.0,                              # prior mean, sd of intercept
    _mu_mu0 = 0.0,
    _mu_tau0 = 1.0,                                # prior mean, sd of mu_l (beta means) (assumed constant over all beta)
    _sigmaa0 = 0.0,
    _sigmab0 = 5.0,                              # limit of uniform for prior shape scale of sigma^2 (model error term variance)
    pa = [1.0, 1.0],
    pb = [1.0, 1.0],
    binits = NaN,
    ...
    @inbounds for i = 1:iter
        ...
        if i > burnin
            ...
            ####################  
            _pi_store[i, :] = _pi
            _sigma_store[i] = sqrt(1.0 / _invsigma2)
            _beta_store[i, :] = _beta
    ...
    nms = vcat(
        ...
        [Symbol("gamma_$i") for i = 1:p],
        [Symbol("bxg_$i") for i = 1:p],
        [Symbol("pi_$i") for i = 1:j],
        :sigma,
    )
    if !isapprox(yscale, 1.0)
        ...
        nms = vcat(nms, [Symbol("b*gu" * "[$i]") for i = 0:p])
    end
    df = DataFrame(rr, nms)
...
gvs_lm(y, X, Xint, pl::Array{Int64,1}, iter::Int64, burnin::Int64; thin = 1, chain = 1) = gvs_lm(
    y,
    X,
    Xint,
    pl,
...
gvs_lm(y, X, Xint, pl::Array{Int64,1}, iter::Int64; thin = 1, chain = 1) =
    gvs_lm(y, X, Xint, pl, iter, 0; thin = thin, chain = chain)
...
function gvs_hlm(
    ...
    chain = 1,
    _mu_eta0 = 0.0,
    _tau_eta0 = 100.0,                              # prior mean, sd of intercept
    _mu_mu0 = [0.0],
    _mu_tau0 = [25.0],                # prior mean, scale of mu_l (beta group means)
    _tau_mu0 = [0.0],
    _tau_tau0 = [25.0],          # prior center, scale of half-t/half-cauchy (_tau_mu0 is unused)
    _sigmaa0 = 0.0,
    _sigmab0 = 5.0,                              # limit of uniform for prior shape scale of sigma^2 (model error term variance)
    pa = [1.0],
    pb = [1.0],                              # prior beta(a,b) parameters for pi (group selection priors)
    binits = NaN,
    nu = 1.0,
    ymean = 1.0, # rescaling coefficients
    yscale = 1.0, # rescaling coefficients
    ...
    nms = vcat(
        ...
        [Symbol("bxg_$i") for i = 1:p],
        [Symbol("pi_$i") for i = 1:j],
        [Symbol("mu" * "[$i]") for i = 1:j],
        [Symbol("tau" * "[$i]") for i = 1:j],
        [Symbol("V" * "[$i]") for i = 1:j],
        [Symbol("alpha" * "[$i]") for i = 1:1],
        :sigma,
    )
    if !isapprox(yscale, 1.0)
        _beta_ustd = hcat(_beta_store[:, 1], _bg_store) .* yscale
...
gvs_hlm(y, X, Xint, pl::Array{Int64,1}, iter::Int64, burnin::Int64; thin = 1, chain = 1, nu = 1.0, fix = true) =
    gvs_hlm(
        y,
        X,
        Xint,
...
gvs_hlm(y, X, Xint, pl::Array{Int64,1}, iter::Int64; thin = 1, chain = 1, nu = 1.0, fix = true) =
    gvs_hlm(y, X, Xint, pl, iter, 0; thin = thin, chain = chain, nu = nu, fix = fix)
...
function gvs_zlm(
    ...
    _tau_eta0 = 10.0,                              # prior mean, sd of intercept
    _mu_mu0 = 0.0,
    _mu_tau0 = 1.0,                            # not actually used here
    _sigmaa0 = 0.0,
    _sigmab0 = 5.0,                              # limit of uniform for prior shape scale of sigma^2 (model error term variance)
    pa = [1.0, 1.0],
    pb = [1.0, 1.0],
    G = 6889.0,                                                     # the g prior
    binits = NaN,
    ymean = 1.0, # rescaling coefficients
    yscale = 1.0, # rescaling coefficients
)
    ...
    # zellner specific 
    _iLams = diagm(vcat(_tau_eta0[1]^(-2), [_mu_tau0[1]^(-2) for j = 1:p])) # this gets overwritten except row/col 1
    xtx = transpose(X) * X
    nu = 1.0
    ####################
    # update loop
...
gvs_zlm(y, X, Xint, pl::Array{Int64,1}, iter::Int64, burnin::Int64; thin = 1, chain = 1, G = 6889.0) = gvs_zlm(
    y,
    X,
    Xint,
    pl,
...
gvs_zlm(y, X, Xint, pl::Array{Int64,1}, iter::Int64; thin = 1, chain = 1, G = 6889.0) =
    gvs_zlm(y, X, Xint, pl, iter, 0; thin = thin, chain = chain, G = G);
