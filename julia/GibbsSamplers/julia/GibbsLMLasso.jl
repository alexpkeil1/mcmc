...
function inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _sigmaa0, binits)
    ...
    _iLams = Diagonal(vcat(_tau_eta0^(-2), fill(0.01, p))) # precision prior on beta coefficients
...
function gibbs_lassolm(
    ...
    _sigmaa0 = 0.0,
    _sigmab0 = 100.0,                                # precision priors
    ymean = 0.0, # rescaling coefficients
    yscale = 1.0, # rescaling coefficients
    _r = 1.0,
    _delta = 1.0,  # lasso penalty shape, rate hyperpriors (conjugate gamma)
)
    ...
    rr = hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, _tau_store, _sig_store, _lambda_store)
    nms = vcat(
        :chain,
        :iter,
        :m1,
...
gibbs_lassolm(y, X, Xint, iter, burnin; thin = 1, chain = 1, ymean = 0.0, yscale = 1.0, _r = 1.0, _delta = 1.0) =
    gibbs_lassolm(
        y,
        X,
        Xint,
...
gibbs_lassolm(y, X, Xint, iter; thin = 1, chain = 1, ymean = 0.0, yscale = 1.0, _r = 1.0, _delta = 1.0) = gibbs_lassolm(
    y,
    X,
    Xint,
    pl,
