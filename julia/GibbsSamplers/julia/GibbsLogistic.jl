...
function inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, binits, pl = NaN)
    ...
    _iLams = Diagonal(vcat(_tau_eta0^(-2), [_tau_beta0^(-2) for j = 1:p])) # precision prior on beta coefficients
...
inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, binits) =
    inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, binits, NaN)
inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0) =
    inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, NaN, NaN)
...
function gibbs_logit(
    ...
    _mu_beta0 = 0.0,
    _tau_beta0 = 100.0,                                # prior mean, sd of beta
)
    (
        N,
        p,
    ...
    res = convert(DataFrame, hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store))
    rename!(res, vcat(:chain, :iter, :m1, :m0, :md, [Symbol("beta" * "[$i]") for i = 0:p]))#, [Symbol("rd" * "$i") for i in 1:nints]))
...
function gibbs_hlogit(
    ...
    _tau_beta0 = 100.0,                                # prior mean, sd of beta
    _tau_mu0 = 0.0,
    _tau_tau0 = 1.0,                                # prior mean, sd of beta
    _mu_mu0 = 0.0,
    _mu_tau0 = 10.0,
)
    (
    ...
    rr = hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, _mu_store, _tausq_store)
    nms = vcat(
        :chain,
        :iter,
        :m1,
...
#  Slice sampler
################################################################################################
function logitbin(_beta, X, y, _mu_eta0 = 0.0, _tau_eta0 = 1000.0, _mu_beta0 = 0.0, _tau_beta0 = 100.0)
...
function slice_logit(
    ...
    println(ap / iter)
    res = DataFrame(hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, __lp))
    rename!(res, vcat(:chain, :iter, :m1, :m0, :md, [Symbol("beta" * "[$i]") for i = 0:p], :__lp))#, [Symbol("rd" * "$i") for i in 1:nints]))
    return res[(burnin+1):iter, :]
end;
