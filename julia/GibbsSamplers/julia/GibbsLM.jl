...
p = 10
X = randn(n,p)
beta = vcat([3,2,1], zeros(p-3))

y = 4.0 .+ X*beta + randn(n)
Xint = [hcat(X[:,1].*0.0 .+ 1.0, X[:,2:end]), hcat(X[:,1].*0.0, X[:,2:end])]
_mu_eta0 = 0.0
_tau_eta0 = 10.0
_mu_beta0 = 0.0
_tau_beta0 = 10.0
...
function inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, _sigmaa0, binits, pl)
    ...
    if isnan(pl[1])
        ...
        plidx = [pls[g]:cumsum(pl)[g]+1 for g = 1:j]
    end
    Ni = size(Xint[1], 1)
    munc, muint = Array{Float64,1}(undef, Ni), Array{Float64,1}(undef, Ni)
    X = hcat(ones(N), X)
...
inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, _sigmaa0, binits = NaN) =
    inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, _sigmaa0, binits, NaN)
inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, _sigmaa0) =
    inits(y, X, Xint, iter, _mu_eta0, _tau_eta0, _mu_beta0, _tau_beta0, _sigmaa0, NaN, NaN)
...
"""
Linear regression model using Gibbs sampling
...
postsamples = gibbs_lm(y,X,[X,X], 1000, 100, MersenneTwister())
hcat(Symbol.(names(postsamples)), [mean(col) for col in eachcol(postsamples)], [std(col) for col in eachcol(postsamples)])

plot(Matrix(postsamples[:,["beta_0", "beta_1", "beta_2", "beta_3", "beta_4"]]), labels=["β0" "β1" "β2" "β3" "β4"])

std(y)
mean(postsamples[:,"sigma"])
```
...
function gibbs_lm(
    ...
    _mu_beta0 = 0.0,
    _tau_beta0 = 1000.0,                              # prior mean, sd of intercept
    _sigmaa0 = 0.0,
    _sigmab0 = 0.0,                                # precision priors
    ...
    rr = hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, _sig_store)
    nms = vcat(:chain, :iter, :m1, :m0, :md, [Symbol("beta_$i") for i = 0:p], :sigma)
    if !isapprox(yscale, 1.0)
        _beta_ustd = _beta_store .* yscale
        _beta_ustd[:, 1] .+= ymean
        rr = hcat(rr, _beta_ustd)
        nms = vcat(nms, [Symbol("betau_$i") for i = 0:p])
    end
    df = DataFrame(rr, nms)
    #exnm = names(df)
...
gibbs_lm(y, X, Xint, iter, burnin; kwargs...) =
    gibbs_lm(y, X, Xint, iter, burnin, MersenneTwister(convert(Int, rand([i for i = 1:1e6]))); kwargs...)
gibbs_lm(y, X, Xint, iter; kwargs...) = gibbs_lm(y, X, Xint, iter, 0; kwargs...);
...
function updatetausq!(rng, _tausq, _iLams, _beta, j, pl, plidx, _mu, _tau_mu0, _tau_tau0)
    @inbounds for q = 1:j # loop over hierarchical groups
        ...
        a_tau = _tau_mu0[q] + pl[q] / 2.0
        b_tau = _tau_tau0[q] + bse / 2.0
        _tausq[q] = rand(rng, Gamma(a_tau, 1.0 / b_tau))
...
function updatev!(rng, _V, _tausq, _tau_tau0, nu, j)
    @inbounds for q = 1:j # loop over hierarchical groups
        _V[q] = rand(rng, InverseGamma((nu + 1.0) / 2.0, _tau_tau0[q]^(-2.0) + nu / _tausq[q]))
    end
end
...
"""
 Hierarchical linear regression model using Gibbs sampling
...
p = 10
X = randn(n,p)
beta = vcat(3.0 .+ randn(div(p,2)), -3.0 .+ randn(p-div(p,2)))

y = 4.0 .+ X*beta + randn(n)



postsamples_lm = gibbs_lm(y,X,[X,X], 1000, 100, MersenneTwister())
postsamples = gibbs_hlm(y,X,[X,X], 1000, 100, MersenneTwister(), pl=[5,5])

hcat(Symbol.(names(postsamples)), [mean(col) for col in eachcol(postsamples)], [std(col) for col in eachcol(postsamples)])

p = plot(Matrix(postsamples[:,["beta_0", "beta_1", "beta_2", "beta_3", "beta_4"]]), labels=["β0" "β1" "β2" "β3" "β4"])
cols = [sp[:linecolor] for sp in p.subplots[1].series_list]
plot!(Matrix(postsamples_lm[:,["beta_0", "beta_1", "beta_2", "beta_3", "beta_4"]]), labels=["β0" "β1" "β2" "β3" "β4"], linecolor=cols)
std(y)
mean(postsamples[:,"sigma"])
```
...
function gibbs_hlm(
    ...
    _sig_store = sqrt.(inv.(_sig_store))
    # this should be new template
    rr = hcat([chain for i = 1:iter], [i for i = 1:iter], _py_store, _beta_store, _mu_store, _tausq_store, _sig_store)
    nms = vcat(
        :chain,
        :iter,
        :m1,
        :m0,
        :md,
        [Symbol("beta_$i") for i = 0:p],
        [Symbol("mu_$i") for i = 1:j],
        [Symbol("tausq_$i") for i = 1:j],
        :sigma,
    )
    if !isapprox(yscale, 1.0)
        _beta_ustd = _beta_store .* yscale
...
gibbs_hlm(y, X, Xint, iter, burnin; kwargs...) =
    gibbs_hlm(y, X, Xint, iter, burnin, MersenneTwister(convert(Int, rand([i for i = 1:1e6]))); kwargs...)
gibbs_hlm(y, X, Xint, iter; kwargs...) = gibbs_hlm(y, X, Xint, iter, 0; kwargs...);
